...
class Vertex(object):
    ...
    def __repr__(self):
        ...
        """
        return 'Vertex(label={}, #incident={}, color={})'.format(self.label, len(self._incidence), self.colornum)
...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        for i in g.vertices:
            myDict[i] = Vertex(ng)
            ng.add_vertex(myDict[i])
        for j in h.vertices:
            myDict[j] = Vertex(ng)
            ng.add_vertex(myDict[j])
        for i in g.edges:
            edge = Edge(myDict[i.tail], myDict[i.head])
            ng.add_edge(edge)
        for j in h.edges:
            edge = Edge(myDict[j.tail], myDict[j.head])
            ng.add_edge(edge)
        return ng
    ...
    def getDict(self):
        ...
        for v in self.vertices:
            if v.colornum in gdict:
                gdict[v.colornum].append(v)
            else:
                gdict[v.colornum] = [v]

        for key in sorted(gdict):
            gdict_sorted[key] = gdict[key]
    ...
    def isConnected(self) -> [bool]:
        pointer = self.vertices[0].label  # Pointer that holds the vertex we are checking
        seen = [False] * len(self.vertices)  # A list of booleans to keep track of which nodes we have seen
        seen[pointer] = True  # Set the first node to True since we start here
        queue = [pointer]

        while queue:  # As long as the breadth is not empty
            pointer = queue.pop(0)  # Take the first element of breadth
            node = self.vertices[pointer]
    ...
    def isTree(self) -> bool:
        # If graph the graph is connected we can do a quick check for a tree
        if self.isConnected():
    ...
    def getTwins(self) -> tuple[list[tuple[Vertex, Vertex]], list[tuple[Vertex, Vertex]]]:
        """
        Returns a list of True and false twins which can help in preparing data for the algorithm
        :return: List of true twins and false twins
        """
        true_twins = []
        false_twins = []
        for v in self.vertices:
            for w in self.vertices:
                if v != w and v.neighbours == w.neighbours:
                    # Check if the reversed edge is already in the list
                    if not ((w, v) in true_twins or (w, v) in false_twins):
                        if v in w.neighbours:
                            true_twins.append((v, w))
                        else:
